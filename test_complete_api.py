...
def test_complete_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            required_summary_fields = ['total_sales', 'total_revenue', 'total_cost', 'total_profit', 'profit_margin']
            missing_summary = [f for f in required_summary_fields if f not in summary]
            
            if not missing_summary:
                print(f"   ✅ Tous les champs du résumé sont présents")
            else:
                print(f"   ❌ Champs manquants dans le résumé: {missing_summary}")
            
            # Vérifier les champs des produits
            if categories:
                first_cat = list(categories.values())[0]
                if first_cat.get('products'):
                    first_product = first_cat['products'][0]
                    required_product_fields = [
                        'name', 'initial_stock', 'incoming', 'quantity_sold', 'final_stock',
                        'unit_price', 'cost_price', 'revenue', 'total_cost', 'profit', 'profit_margin'
                    ]
                    missing_product = [f for f in required_product_fields if f not in first_product]
                    
                    if not missing_product:
                        print(f"   ✅ Tous les champs des produits sont présents")
                    else:
                        print(f"   ❌ Champs manquants dans les produits: {missing_product}")
