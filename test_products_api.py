    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if products:
                ...
                products_with_zero_purchase = len([p for p in products if p.get('purchase_price') == 0])
                products_with_valid_purchase = len([p for p in products if p.get('purchase_price', 0) > 0])
...
def test_single_product():
    ...
    try:
        # Essayer de récupérer le produit ID 1
        ...
        if response.status_code == 200:
            ...
            print(f"   • purchase_price: {product.get('purchase_price', 'MANQUANT')}")
            print(f"   • selling_price: {product.get('selling_price', 'N/A')}")
            
            return product.get('purchase_price') is not None
        else:
