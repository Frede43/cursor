#!/usr/bin/env python
"""
Script pour créer et tester un utilisateur avec permissions spécifiques
Utilise Django ORM avec la bonne configuration
"""
...
def demonstrate_frontend_integration():
    """Démontrer comment le frontend utiliserait ces permissions"""
    print(f"\n🌐 Intégration Frontend - Exemple de code React:")
    print(f"""
    ...
    return (
      <nav>
        ...
        {{/* Visible si permission sales.create */}}
        {{hasPermission('sales.create') && (
          <NavItem to="/sales" icon={{ShoppingCart}}>
            Ventes (POS)
          </NavItem>
        )}}
        
        {{/* Visible si permission sales.view */}}
        {{hasPermission('sales.view') && (
          <NavItem to="/sales-list" icon={{List}}>
            Liste des Ventes
          </NavItem>
        )}}
        
        {{/* Visible si permission sales.history */}}
        {{hasPermission('sales.history') && (
          <NavItem to="/sales-history" icon={{History}}>
            Historique des Ventes
          </NavItem>
        )}}
        
        {{/* Non visible - pas de permissions */}}
...
def main():
    ...
    try:
        ...
        if auth_user:
            ...
            # 7. Démonstration d'intégration frontend
            ...
            print(f"   • Se connecter au système BarStockWise")
            print(f"   • Accéder au module de ventes (POS)")
            print(f"   • Créer de nouvelles ventes")
            print(f"   • Consulter l'historique des ventes")
            print(f"   • Voir la liste des ventes")
            
            print(f"\n🚫 L'utilisateur NE PEUT PAS:")
            print(f"   • Gérer les produits")
            print(f"   • Accéder aux stocks")
            print(f"   • Gérer les utilisateurs")
            print(f"   • Modifier les paramètres")
